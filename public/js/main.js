...
async function fetchData(url, options = {}) {
    try {
        ...
        if (window.location.pathname === '/login' && !document.getElementById(msgElementId)) {
            // Do not display fetch error on login page unless specific area exists
        } else {
...
async function handleLogout() {
    ...
    try {
        ...
        if (response.ok && response.redirected) {
            ...
            const errorText = await response.text(); alert('登出失败: ' + errorText);
        }
    } catch (error) { alert('登出时发生错误。'); }
}
...
async function loadNotes() {
    ...
    if (notes.length === 0) {
        notesContainer.innerHTML = '<p>当前没有记事。 <a href="/note/new" class="button-action">创建您的第一篇记事！</a></p>';
        return;
    }
    ...
    notes.forEach(note => {
        ...
        li.id = `note-${note.id}`;
        let ownerInfo = note.ownerUsername ? `<span class="note-owner">(所有者: ${escapeHtml(note.ownerUsername)})</span>` : '';
        ...
        if (note.attachment && note.attachment.path) {
            ...
            attachmentHtml = `
                <div class="note-attachment">
                    附件: <a href="${attachmentUrl}" target="_blank" title="下载 ${escapeHtml(note.attachment.originalName)} (${(note.attachment.size / 1024).toFixed(1)} KB)">
